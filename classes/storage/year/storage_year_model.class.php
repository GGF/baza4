...
class storage_year_model extends storage_rest_model {
    ...
    public function arc() {
        ...
        foreach ($res as $rs) {
            ...
            // коментарий
            $sql = "SELECT id FROM {$this->db}coments WHERE comment='ѕередача остатка'";
            $rs1 = sql::fetchOne($sql);
            if (!empty($rs1)) {
                $comment_id = $rs1["id"];
            } else {
                $sql = "INSERT INTO {$this->db}coments (comment) VALUES ('ѕередача остатка')";
            ...
            // коментарий
            $sql = "SELECT id FROM {$this->db}coments WHERE comment='ќстаток на 31.12.$docyr'";
            $rs1 = sql::fetchOne($sql);
            if (!empty($rs1)) {
                $comment_id = $rs1["id"];
            } else {
                $sql = "INSERT INTO {$this->db}coments (comment) VALUES ('ќстаток на 31.12.$docyr')";
                sql::query($sql) or die(sql::error(true));
                $comment_id = sql::lastId();
            }
            $docyr = date("Y");
