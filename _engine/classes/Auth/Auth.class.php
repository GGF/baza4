...
class Auth extends lego_abstract {
    ...
    public function __construct($name=false, $directCall = true) {
        if ($directCall) {
            trigger_error("Ќельз€ использовать конструктор дл€" .
                    "создани€ класса console." .
                    "»спользуйте статический метод getInstance()", E_USER_ERROR);
        }
    ...
    public function action_index() {
        ...
        if (!empty($sessionid)) {
            ...
            if (!empty($rs)) {
                ...
                if (!empty($rs)) {
                    ...
                    $mes .= "Ќе могу найти пользовател€ по сессии." .
                            "ќбратитесь к разработчику!";
                }
            } else {
                $mes .= "—есси€ не верна или устарела!";
            }
        }

        // пуста€ сесси€, не восстановлена о базе, не найден пользователь
        ...
        Output::assign('title', 'јвторизаци€');
