...
function sk_parse_config_str($str, $is_unicode = false) {
	...
	if ($is_unicode) {
		...
		$str = preg_replace_callback($tmppat,function($in) { return sl_screenspecial($in[1], true);}, $str);
	} else {
		//$str = preg_replace_callback($tmppat,create_function('$in', 'return sl_screenspecial($in[1], false);'), $str);
		$str = preg_replace_callback($tmppat,function($in){return sl_screenspecial($in[1], false);}, $str);
	}
