<?php
/**
 * This file provides the main QR_Code class.
 * 
 * The project is a rewrite from jQuery extension available at
 * @link https://github.com/jeromeetienne/jquery-qrcode
 * 
 * For a detailed description of QR_Code and its features please check
 * @link http://www.qrcode.com/
 * 
 * QR_ Code is registered trademark of
 ...
 */
//require_once 'QR_8bitByte.php';
//require_once 'QR_BitBuffer.php';
//require_once 'QR_RSBlock.php';
 ...
 *
 * It provides the functionality to generate a QR_Code bitmap
...
class QR_Code
{
  ...
  /**
   * Create a new instance of QR_Code
   * 
  ...
  /**
   * Add a new data element to the QR_Code
   *
   * @param string $data          
   */
  public function addData($data)
  {
    $newData = new QR_8bitByte ( $data );
  ...
  public function isDark($row, $col)
  {
    ...
    {
      throw new QR_CodeException ( "$row,$col" );
    }
  ...
  public function make()
  {
    if ($this->typeNumber < 1)
    {
      ...
      {
        $rsBlocks = QR_RSBlock::getInstance ()->getRSBlocks ( $typeNumber, $this->errorCorrectLevel );
  ...
  private function makeImpl($test, $maskPattern)
  {
    ...
    $this->modules = QR_Util::getInstance ()->createEmptyArray ( $this->moduleCount );
    
    for($row = 0; $row < $this->moduleCount; $row ++)
    {
      $this->modules [$row] = QR_Util::getInstance ()->createEmptyArray ( $this->moduleCount );
  ...
  /**
   * Get the best mask pattern for this QR_Code
   *
   * @return int
   */
  private function getBestMaskPattern()
  {
    ...
    for($i = 0; $i < 8; $i ++)
    {
      ...
      $lostPoint = QR_Util::getInstance ()->getLostPoint ( $this );
  ...
  {
    $pos = QR_Util::getInstance ()->getPatternPosition ( $this->typeNumber );
  ...
  {
    $bits = QR_Util::getInstance ()->getBCHTypeNumber ( $this->typeNumber );
  ...
  private function setupTypeInfo($test, $maskPattern)
  {
    ...
    $bits = QR_Util::getInstance ()->getBCHTypeInfo ( $data );
      ...
      while ( true )
      {
        for($c = 0; $c < 2; $c ++)
        {
          if ($this->modules [$row] [$col - $c] === null)
          {
            ...
            $mask = QR_Util::getInstance ()->getMask ( $maskPattern, $row, $col - $c );
