...
//require_once 'QR_Polynominal.php';
 ...
 * @package phpQr
 */
class QR_UtilException extends QR_CodeException {};
 ...
 * functionality for the QR_Code class and its parts.
 *  
 * @author Maik Greubel <greubel@nkey.de>
 * @package phpQr
 */
class QR_Util
{
  ...
  /**
   * Create a new instance of QR_Util
   */
  ...
  public function getMask($maskPattern, $i, $j)
  {
    switch($maskPattern)
    {
      case QR_MaskPattern::PATTERN000: return ($i + $j) % 2 == 0;
      case QR_MaskPattern::PATTERN001: return ($i % 2) == 0;
      case QR_MaskPattern::PATTERN010: return ($j % 3) == 0;
      case QR_MaskPattern::PATTERN011: return ($i + $j) % 3 == 0;
      case QR_MaskPattern::PATTERN100: return (floor($i / 2) + floor($j / 3)) % 2 == 0;
      case QR_MaskPattern::PATTERN101: return ($i * $j) % 2 + ($i * $j) % 3 == 0;
      case QR_MaskPattern::PATTERN110: return (($i * $j) % 2 + ($i * $j) % 3) % 2 == 0;
      case QR_MaskPattern::PATTERN111: return (($i * $j) % 3 + ($i + $j) % 2) % 2 == 0;
      
      default: throw new QR_UtilException("Bad mask pattern " . $maskPattern); 
    }
  }
  ...
  public function getLengthInBits($mode, $type)
  {
    ...
    if(1 <= $type && $type < 10)
    {
      switch($mode)
      {
        ...
        default: throw new QR_UtilException("Bad mode " . $mode);
      }
    }
    // 10 - 26
    else if($type < 27)
    ...
    {
      throw new QR_UtilException("Bad type " . $type);
    }
  }
