...
//require_once 'QR_ErrorCorrectLevel.php';
 ...
 * @package phpQr
 */
class QR_RSBlockException extends QR_CodeException
{ 
}

/**
 * This class is a Reed-Solomon implementation for the QR_Code.
...
class QR_RSBlock
{
  ...
  /**
   * Create a new QR_ Reed-Solomon block instance
   * 
  ...
  public function getRSBlocks($typeNumber, $errorCorrectLevel)
  {
    ...
    if(!$rsBlock)
    {
      throw new QR_RSBlockException("Bad RS Block at type number " . $typeNumber . " / error correct level " . $errorCorrectLevel);
    }
    ...
    for($i = 0; $i < $length; $i++)
    {
      ...
      for($j = 0; $j < $count; $j++)
      {
        array_push($list, new QR_RSBlock($totalCount, $dataCount));
      }
    }
