 ...
 */
    function __construct($type, $array) {
        ...
        $this->_connection = 
                $connect($array[host], $array[name], $array[pass], true);
        if (!$this->_connection) 
                throw(new Exception("Ошибка подключения: " . mysql_error()));
...
//                collation_connection='{$encoding}_GENERAL_CI'");
            ...
            throw(new Exception("Ошибка выбора бд: " . $this->error()));
    }
    ...
    function format($query) {
        ...
        return str_replace($this->_PREGtokens[patterns], 
                $this->_PREGtokens[replaces], $query);
    }
     ...
     * использует его, если не передано ничего — берет последний 
     * записанный ресурс — небезопасно
     * 	@param	array	$array	
     * Массив для подстановки в запрос (также тут может быть и сразу тип лога,
     ...
     * (также тут может быть и сразу тип лога, 
     * см. функцию query() — это DEPRECATED!)
     * 	@param	int		$i	Номер поля для возврата
     * 	@param	bool		$log	Тип лога, см. функцию query()
     * 	@return	array
     ...
     * делает запрос, если передан ресурс — использует его, 
     * если не передано ничего — берет последний записанный 
     * ресурс — небезопасно
     * 	@param	array	$array	Массив для подстановки в запрос 
     * (также тут может быть и сразу тип лога, 
     * см. функцию query() — это DEPRECATED!)
     ...
     * сделан ссылкой для экономии памяти
     * 	@return array
     */
